...
class LossSigmoid(nn.Module):
    ...
    def forward(self, logits, labells, dec):
        ...
        labels = (2.0 * labells - 1.0).unsqueeze(-1)  
        
        label_all = torch.cat([-1 * labels, labels], dim=1) 
        input_sig = label_all.unsqueeze(1) * logits 
        loss_each = self.log_sigmoid(input_sig) 
        if torch.isnan(loss_each).any():
            print("NaN in loss")

        dec_each_in = dec(loss_each)  
...
class Loss_detection(nn.Module):
    def __init__(self,args, device, loss_type="sigmoid", dec_type="mean", lr=0.001):
        ...
        if dec_type == "mean":
            ...
            self.alphadec = (torch.ones(1) * 0.0).to(device)  
